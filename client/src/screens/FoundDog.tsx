  ...
  return (
    <div className="flex flex-col h-full bg-bg">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 flex flex-col gap-4">
        <div className="bg-surface border border-amber/15 rounded-2xl border-2 border-dashed border-[#2c3540] rounded-2xl py-10 flex flex-col items-center gap-3 cursor-pointer"
          ...
          <p className="text-xs text-muted text-center max-w-[200px] leading-relaxed">No app needed — hold phone to dog's shoulder</p>
        </div>
        ...
        <div className="flex gap-2">
          <input value={id} onChange={e=>setId(e.target.value)} placeholder="985000012384721"
            className="flex-1 bg-surface border border-amber/15 rounded-2xl border border-[#2c3540] rounded-xl px-3.5 py-3 text-sm font-mono text-amber outline-none focus:border-amber placeholder:text-muted tracking-[.06em]"/>
        ...
        {result && (
          <div className="bg-green/5 border border-green/30 rounded-2xl p-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div><p className="font-mono text-[8px] text-muted uppercase">Breed</p><p className="font-semibold mt-0.5">{result.breed}</p></div>
              <div><p className="font-mono text-[8px] text-muted uppercase">Owner</p><p className="font-semibold mt-0.5">{result.owner}</p></div>
              <div><p className="font-mono text-[8px] text-muted uppercase">Contact</p><p className="font-semibold mt-0.5 text-amber">{result.contact}</p></div>
            </div>
            <div className="flex gap-2">
        ...
        <div className="bg-surface border border-amber/15 rounded-2xl border border-amber/15 rounded-2xl overflow-hidden">
          <p className="px-4 py-3 text-xs font-semibold text-muted border-b border-amber/15">If dog has no chip</p>
          <button className="w-full px-4 py-3.5 text-left text-sm border-b border-amber/15 flex items-center gap-3" onClick={() => showToast('Sighting reporter opened')}>
