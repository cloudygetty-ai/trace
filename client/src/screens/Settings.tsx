...
function SettingRow({ icon, title, sub, right }: any) {
  return (
    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border last:border-none">
      <div className="w-8 h-8 rounded-xl bg-s2 border border-border flex items-center justify-center text-base flex-shrink-0">{icon}</div>
...
function Toggle({ on, toggle }: { on: boolean; toggle: () => void }) {
  return (
    <div onClick={toggle} className={"w-11 h-6 rounded-full relative cursor-pointer transition-all flex-shrink-0 " + (on ? 'bg-cyan' : 'bg-s3 border border-[#2c3540]')}>
    ...
    <div className="flex flex-col h-full bg-bg">
      ...
      <div className="flex-1 overflow-y-auto pb-8">
        ...
        <div className="px-4 py-5 flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-cyan/10 border-2 border-cyan/30 flex items-center justify-center text-3xl">👤</div>
          <div>
            ...
            <p className="font-mono text-[9px] text-cyan mt-1.5 tracking-[.06em]">{dogs.length} DOGS · ACCT MEMBER</p>
          </div>
        </div>

        <div className="mx-4 mb-2 font-mono text-[9px] text-muted2 uppercase tracking-[.1em]">Alert Preferences</div>
        <div className="mx-4 bg-s1 border border-border rounded-2xl overflow-hidden">
        ...
        <div className="px-4 mt-6">
          <button onClick={async () => { await signOut(); nav('/login'); }}
            className="w-full h-11 bg-transparent border border-border rounded-xl font-mono text-[11px] text-muted2 tracking-[.06em]">
            Sign Out
          </button>
