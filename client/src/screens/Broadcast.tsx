      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-4">
        {dog && (
          <div className="bg-warn/5 border border-warn/30 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Last seen 4 hours ago</p>
            </div>
          </div>
        )}
        <div className="bg-surface border border-amber/15 rounded-2xl border border-amber/15 rounded-2xl overflow-hidden">
          {[
            ...
            <div key={r.label} className="flex items-center gap-3 px-4 py-4 border-b border-amber/15 last:border-none">
              <span className="text-xl">{r.icon}</span>
              <div className="flex-1"><p className="text-sm font-medium">{r.label}</p><p className="text-xs text-muted mt-0.5">{r.sub}</p></div>
              <Toggle on={r.on} onToggle={r.toggle}/>
            </div>
          ))}
          <div className="px-4 py-4 border-t border-amber/15 flex items-center gap-3">
        ...
        <div>
          <label className="font-mono text-[9px] uppercase tracking-[.1em] text-muted">Alert radius</label>
          <input type="range" min={1} max={10} step={0.5} value={radius} onChange={e=>setRadius(+e.target.value)}
            className="w-full mt-2 accent-cyan"/>
          <p className="font-mono text-lg font-bold text-amber text-center mt-1">{radius} mi</p>
        </div>
        <Btn full onClick={send} disabled={sending}>{sending ? 'Sending...' : '🚨  Send Alert Now'}</Btn>
        <p className="text-[11px] text-muted text-center leading-relaxed">Only opted-in recipients receive alerts. All messages include STOP instructions.</p>
      </div>
    </div>
  );
}
