    ...
    <div className="flex flex-col h-full bg-bg">
      ...
      <div className="flex-1 relative overflow-hidden bg-[#090c0b]">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2">
          <div className="bg-[rgba(12,15,16,.88)] border border-amber/15 rounded-xl p-2.5 px-3 backdrop-blur-md">
          ...
          <div className="bg-[rgba(12,15,16,.88)] border border-amber/15 rounded-xl p-2.5 px-3 backdrop-blur-md">
            ...
            <p className="font-mono text-[8px] text-muted tracking-[.06em] mt-1">LAST SEEN</p>
          </div>
        </div>
      </div>
      <div className="bg-surface border border-amber/15 rounded-2xl border-t border-amber/15 p-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          <span className="text-xl">🐕</span>
          <p className="text-sm font-semibold flex-1">{activeDog?.name ?? 'Luna'} — <span className="text-warn text-xs">LOST</span></p>
          <button onClick={() => nav('/broadcast')} className="h-9 px-4 bg-transparent border border-amber/15 rounded-xl font-mono text-[9px] text-muted">📢 Broadcast</button>
        </div>
        <div className="flex gap-2 mt-3 flex-wrap">
          {[['bg-amber','BLE relay'],['bg-purple-400','Community'],['bg-amber-400','Last seen']].map(([c,l]) => (
            <div key={l} className="bg-surface border border-amber/15 rounded-2xl border border-amber/15 rounded-xl px-2.5 py-1.5 flex items-center gap-1.5">
